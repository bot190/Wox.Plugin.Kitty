...
class Kitty(Wox):
        ...
        if session_location == "file":
            ...
            for root, directories, filenames in walk(session_path):
                for filename in filenames:
                    sessionlist.append(
                        (
                            root.replace(session_path, "", 1).replace("\\", ""),
            ...
            sessions = []
            key = winreg.OpenKey(
                winreg.HKEY_CURRENT_USER, r"Software\9bis.com\KiTTY\Sessions"
            )
                ...
                res.append(
                    {
                        ...
                        "JsonRPCAction": {
                            "method": "open_session",
                            "parameters": [kitty_path.replace("\\", "\\\\"), dir, p],
                        },
                    }
                )
        if len(res) == 0:
    ...
    def open_session(self, kitty_path, folder_name, session_name):
        if folder_name is not "":
            subprocess.Popen(
                '{} -folder "{}" -load "{}"'.format(
                    kitty_path, folder_name, session_name
                )
            )
        else:
